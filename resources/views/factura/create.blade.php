                            ...
                            <div class="col-md-4">
                                ...
                                <div class="mb-3">
                                    <input type="text" id="fact_nro" name="fact_nro" class="form-control" placeholder="Ingrese nro de factura" required>
                                        <small id="factura-msg" class="fs-5"></small>
                                </div>
                            </div>
                            <div class="col-md-3">
...
document.getElementById('fact_nro').addEventListener('blur', function () {

    const nro = this.value;
    if(!nro) return;

    fetch(`{{ route('facturas.validar') }}?fact_nro=${nro}`)
        .then(res => res.json())
        .then(data => {
            ...
            if(data.exists){
                numeroValido = false;
                msg.innerText = "Este número de factura ya existe.";
                msg.style.color = "red";
            } else {
                numeroValido = true;
                msg.innerText = "Número disponible.";
                msg.style.color = "green";
            }
        })
        .catch(() => {
            console.error("Error validando número de factura");
        });
});
